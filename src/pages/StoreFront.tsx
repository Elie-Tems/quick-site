...
const StoreFront = ({ slugOverride }: { slugOverride?: string } = {}) => {
  ...
  useEffect(() => {
    function handleDesignPreview(event: MessageEvent) {
      ...
      if (primaryColor && /^#[0-9a-fA-F]{6}$/.test(primaryColor)) {
        // Convert hex to HSL — the format Tailwind CSS variables expect (e.g. "142 71% 45%")
  ...
  const layoutProps: StorefrontLayoutProps = {
    ...
    ctaText: (() => {
      const raw = (business.cta_text as string | null | undefined) || undefined;
      const layoutId = template?.layoutId ?? getDefaultLayout(business);
      const catDefault = getCategoryConfig(businessCategory).ctaText;
      // Type-aware defaults - AI-generated CTAs are often wrong for the vertical.
      ...
      // A generic clothing CTA ("קולקציה") on a non-fashion category is wrong - use the
      // category-appropriate default instead (e.g. beauty -> "לחנות").
      const FASHION = new Set(['clothing', 'jewelry', 'home', 'furniture']);
      if (raw && /קולקציה/.test(raw) && !FASHION.has(businessCategory as string)) return catDefault;
      return raw || catDefault;
    })(),
