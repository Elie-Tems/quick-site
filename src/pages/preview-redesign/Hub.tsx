...
import { AuroraBg, Card } from "@/components/preview-redesign/kit";

/** Hub linking every redesign preview. Route: /preview/redesign */

const SCREENS = [
  { to: "/preview/home-v2", icon: Home, title: "דף הבית", desc: "הדף השיווקי - אנימציות, וידאו, אנרגיה צעירה", tag: "מאושר ✓", tone: "green" },
      ...
      <div className="max-w-4xl mx-auto px-4 py-16 md:py-24">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center mb-14">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/30 mb-6">
            ...
            <span className="text-sm text-white/80">קו עיצוב חדש - תצוגה מקדימה</span>
          </div>
          <h1 className="text-4xl md:text-6xl font-display font-bold mb-4">
            העיצוב החדש של <span className="bg-gradient-to-l from-primary via-emerald-400 to-lime-300 bg-clip-text text-transparent">Siango</span>
          </h1>
          <p className="text-lg text-white/60 max-w-xl mx-auto">
            כל המסכים בשפה אחת - צעירה, נקייה ואנימטיבית. מוקאפים בלבד; המערכת האמיתית לא השתנתה.
          </p>
        </motion.div>
              ...
              <Link to={s.to}>
                <Card hover className="p-6 h-full group relative overflow-hidden">
                  ...
                  <div className="relative flex items-start justify-between mb-4">
                    ...
                    <span className={`text-xs font-medium px-2.5 py-1 rounded-full border ${s.tone === "green" ? "bg-emerald-500/15 text-emerald-300 border-emerald-500/30" : "bg-primary/15 text-primary border-primary/30"}`}>{s.tag}</span>
                  </div>
                  <h3 className="relative text-xl font-display font-bold text-white mb-1.5 flex items-center gap-2">
                    {s.title}
                  ...
                  <p className="relative text-sm text-white/50 leading-relaxed">{s.desc}</p>
                </Card>
              </Link>
            </motion.div>
          ))}
        ...
        <p className="text-center text-white/30 text-sm mt-12">
          לחיצה על כל כרטיס פותחת את המסך המלא. יש כפתור "חזרה לכל המסכים" בכל מוקאפ.
        </p>
      </div>
    </div>
  );
