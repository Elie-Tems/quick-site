...
const Dashboard = () => {
  ...
  // The "יומן ולידים" nav entry + its view show only when a non-commerce module is
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case 'profitability':
        return (
          <PremiumOverlay
            ...
            description="דעו מי הלקוחות שמכניסים לכם הכי הרבה כסף, מתי להחזיר לקוח שנעלם, ומה הרווח האמיתי אחרי עלויות."
            bullets={["גלו מי הלקוח שקנה הכי הרבה — ופנו אליו בזמן הנכון", "שלחו הטבה בוואטסאפ ללקוחות שלא חזרו כבר חודש", "ראו לכמה עולה לכם כל מוצר אחרי תשלום לספק", "גרף רווחיות חודשי — כדי לדעת האם העסק צומח"]}
      ...
      case 'about':
      case 'content':
        return <DashboardContent businessId={business?.id} businessType={getBusinessType(business)} businessSubType={(business as any)?.business_sub_type} onNavigate={goToView} />;
      case 'shipping':
        return (
