...
const EmailCard = ({ businessId, def, override }: {
  ...
  const doSave = () =>
    save.mutate({ businessId, templateKey: def.key, ...draft },
      { onSuccess: () => toast.success("נשמר"), onError: () => toast.error("שמירה נכשלה") });

  const doReset = () =>
    reset.mutate({ businessId, templateKey: def.key }, {
      onSuccess: () => { setDraft({ subject: def.defaults.subject, heading: def.defaults.heading, body: def.defaults.body, button_text: def.defaults.button ?? "" }); toast.success("שוחזר לברירת מחדל"); },
    });
    ...
    <div className="rounded-xl border border-border bg-card">
      <div className="flex items-center gap-3 p-3.5">
        ...
        <button onClick={() => setOpen((o) => !o)} className="flex-1 text-right">
          <div className="font-medium text-foreground flex items-center gap-1.5">{def.label} <ChevronDown className={`w-3.5 h-3.5 text-muted-foreground transition-transform ${open ? "rotate-180" : ""}`} /></div>
          <div className="text-xs text-muted-foreground truncate">{override?.subject ?? def.defaults.subject}</div>
        </button>
        {def.cancellable ? (
          <button onClick={() => save.mutate({ businessId, templateKey: def.key, enabled: !enabled }, { onSuccess: () => toast.success(enabled ? "המייל כבוי" : "המייל פעיל") })}
          ...
          <span className="text-[11px] text-muted-foreground shrink-0">תמיד נשלח</span>
        )}
      </div>
      ...
      {open && (
        <div className="px-3.5 pb-3.5 space-y-2 border-t border-border pt-3">
          <div>
            <label className="text-xs text-muted-foreground">כותרת המייל (subject)</label>
            <Input value={draft.subject} onChange={(e) => setDraft({ ...draft, subject: e.target.value })} />
          </div>
          <div>
            <label className="text-xs text-muted-foreground">כותרת ראשית</label>
            <Input value={draft.heading} onChange={(e) => setDraft({ ...draft, heading: e.target.value })} />
          </div>
          <div>
            <label className="text-xs text-muted-foreground">טקסט (אפשר {"{name}"} לשם הלקוח)</label>
            <Textarea rows={3} value={draft.body} onChange={(e) => setDraft({ ...draft, body: e.target.value })} />
          </div>
          <div>
            <label className="text-xs text-muted-foreground">כפתור (ריק = בלי כפתור)</label>
            <Input value={draft.button_text} onChange={(e) => setDraft({ ...draft, button_text: e.target.value })} />
          </div>
          <div className="flex items-center gap-2 pt-1">
            <Button size="sm" onClick={doSave} disabled={save.isPending} className="gap-1.5"><Check className="w-4 h-4" /> שמור</Button>
            <Button size="sm" variant="outline" onClick={doReset} disabled={reset.isPending} className="gap-1.5"><RotateCcw className="w-3.5 h-3.5" /> ברירת מחדל</Button>
          </div>
        </div>
      )}
    </div>
...
const LifecycleEmailsManager = ({ business }: { business: (BusinessLike & { id: string }) | null | undefined }) => {
  ...
  return (
    <section>
      <h3 className="text-lg font-bold text-foreground mb-1 flex items-center gap-2"><Mail className="w-5 h-5 text-primary" /> מיילים אוטומטיים ללקוחות</h3>
      <p className="text-sm text-muted-foreground mb-3">כל מייל נשלח עם הלוגו והצבעים שלכם. אפשר לכבות מיילים או לערוך את הטקסט.</p>
