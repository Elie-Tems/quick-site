  ...
  return (
    <div className="p-4 md:p-6 space-y-6 w-full">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Storefront Texts Section */}
        <div className="bg-card rounded-xl border border-border p-5 shadow-soft space-y-5">
          <button
            type="button"
            className="w-full flex items-center justify-between text-right"
            onClick={() => setShowStoreTexts(prev => !prev)}
          >
            <div className="flex items-center gap-3">
              <Type className="h-5 w-5 text-muted-foreground" />
              <div className="flex flex-col items-start">
                <h2 className="font-semibold text-lg text-foreground">טקסטים בחנות</h2>
                <p className="text-xs text-muted-foreground">
                  התאם את הטקסטים המוצגים ללקוחות בחנות שלך
                </p>
              </div>
            </div>
            {showStoreTexts ? (
            ...
            <div className="space-y-5 pt-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-muted-foreground">
                      {effectiveUseHeroTitle ? "מופעל" : "ללא כותרת מותאמת"}
                    </span>
                    <Switch
                      checked={effectiveUseHeroTitle}
                      onCheckedChange={(checked) =>
                        setFormData({ ...formData, useHeroTitle: checked })
                      }
                    />
                  </div>
                </div>
                <Input
                  ...
                  placeholder={formData.name || "שם העסק יוצג כברירת מחדל"}
                  disabled={!effectiveUseHeroTitle}
                />
                <p className="text-xs text-muted-foreground">
                  הכותרת הגדולה בראש החנות. השאר ריק להצגת שם העסק
                </p>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-muted-foreground">
                      {effectiveUseHeroBadge ? "מופעל" : "ללא תג"}
                    </span>
                    <Switch
                      checked={effectiveUseHeroBadge}
                      onCheckedChange={(checked) =>
                        setFormData({ ...formData, useHeroBadge: checked })
                      }
                    />
                  </div>
                </div>
                <Input
                  ...
                  disabled={!effectiveUseHeroBadge}
                />
                <p className="text-xs text-muted-foreground">
                  תג קטן שמופיע מעל הכותרת הראשית (לדוגמה: "מבצע!", "חדש בחנות")
                </p>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-muted-foreground">
                      {effectiveUsePromoText ? "מופעל" : "ללא פרומו"}
                    </span>
                    <Switch
                      checked={effectiveUsePromoText}
                      onCheckedChange={(checked) =>
                        setFormData({ ...formData, usePromoText: checked })
                      }
                    />
                  </div>
                </div>
                <Textarea
                  ...
                  rows={2}
                  disabled={!effectiveUsePromoText}
                />
                <p className="text-xs text-muted-foreground">
                  הודעה שמופיעה בפס הסגול בראש החנות
                </p>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-muted-foreground">
                      {effectiveUseCtaText ? "מופעל" : "טקסט דיפולטי"}
                    </span>
                    <Switch
                      checked={effectiveUseCtaText}
                      onCheckedChange={(checked) =>
                        setFormData({ ...formData, useCtaText: checked })
                      }
                    />
                  </div>
                </div>
                <Input
                  ...
                  disabled={!effectiveUseCtaText}
                />
                <p className="text-xs text-muted-foreground">
                  הטקסט בכפתור הראשי שמוביל למוצרים
                </p>
              </div>

              {/* Hero info strip (benefits below banner) */}
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <Label className="flex items-center gap-2">פס מידע מתחת לבאנר</Label>
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-muted-foreground">
                      {effectiveUseHeroBenefits ? "מופעל" : "מוסתר"}
                    </span>
                    <Switch
                      checked={effectiveUseHeroBenefits}
                      onCheckedChange={(checked) =>
                        setFormData({ ...formData, useHeroBenefits: checked })
                      }
                    />
                  </div>
                </div>
                <p className="text-xs text-muted-foreground">
                  עד 3 שורות שמופיעות מתחת ל-Hero (למשל: משלוח חינם, החזרות, תשלום מאובטח). השאר ריק כדי להציג ברירת מחדל.
                </p>
                {[0, 1, 2].map((i) => (
                  <Input
                    key={i}
                    value={formData.heroBenefits?.[i] ?? ''}
                    onChange={(e) => {
                      const next = [...(formData.heroBenefits ?? ['', '', ''])];
                      next[i] = e.target.value;
                      setFormData({ ...formData, heroBenefits: next });
                    }}
                    placeholder={
                      i === 0
                        ? '🚚 משלוח חינם מעל ₪199'
                    ...
                    disabled={!effectiveUseHeroBenefits}
                  />
                ))}
              </div>
            </div>
        ...
        <div className="bg-card rounded-xl border border-border p-5 shadow-soft space-y-5">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col items-start">
                <h2 className="font-semibold text-lg text-foreground">צבעים ותמונות רקע</h2>
              </div>
            </div>
            {showBranding ? (
              <ChevronUp className="h-5 w-5 text-muted-foreground" />
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-muted-foreground">הצבע הראשי יופיע בכפתורים, קישורים ואלמנטים בולטים</p>
          </div>
          {/* Hero Image */}
          <div className="space-y-4">
            ...
            <p className="text-xs text-muted-foreground">
              העלה תמונה מהמחשב (עד 5MB) או הדבק קישור לתמונה
            </p>
            ...
            {/* AI Hero Generation */}
            <div className="p-4 rounded-lg bg-gradient-to-r from-primary/5 to-purple-500/5 border border-primary/20 space-y-4">
          ...
          <div className="space-y-3">
            ...
            <RadioGroup
              ...
              onValueChange={(value) => setFormData({ ...formData, brandStyle: value as BusinessSettings['brandStyle'] })}
            ...
            >
              {brandStyles.map((style) => (
                <Label
                  ...
                  className={`flex flex-col justify-center gap-1 p-4 rounded-lg border cursor-pointer transition-colors h-[4.5rem] overflow-visible ${
                    formData.brandStyle === style.id 
                      ? 'border-primary' 
                      : 'border-border hover:border-primary/30'
                  }`}
                >
                  <RadioGroupItem value={style.id} id={`style-${style.id}`} className="sr-only absolute opacity-0 pointer-events-none w-0 h-0" />
                  <div className="flex flex-col gap-0.5 min-w-0">
                    <span className="font-medium text-foreground truncate">{style.name}</span>
                    <span className="text-xs text-muted-foreground truncate">{style.description}</span>
                  </div>
                </Label>
              ))}
            </RadioGroup>
              ...
              <div className="space-y-3">
                <Label className="text-sm">בחר קטגוריית עסק ליצירת תמונה מותאמת:</Label>
                <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                  {categories.map((cat) => (
                    <button
                      key={cat.id}
              ...
              <div className="space-y-2">
                ...
                >
                  {showCustomPrompt ? "תארו את התמונה שתרצו ליצור (סגנון, צבעים, דמויות, רקע וכו')" : "תארו את התמונה שתרצו ליצור (סגנון, צבעים, דמויות, רקע וכו')"}
                </button>
                {showCustomPrompt && (
                  <Input
              ...
              <Button
                ...
                disabled={isGeneratingHero || !settings.id}
                className="w-full gap-2"
              >
                {isGeneratingHero ? (
                  <>
                  ...
                  <>
                    ...
                    {formData.heroImageUrl ? 'יצירה מחדש' : 'צור תמונה ראשית לאתר'}
                  </>
                )}
              </Button>
              
              <p className="text-xs text-muted-foreground">
                המערכת תייצר תמונה מקצועית המותאמת לסוג העסק שלך
              </p>
            </div>
            
            <p className="text-xs text-muted-foreground">תמונה רחבה לבאנר הראשי בחנות (מומלץ: 1920x1080)</p>
          </div>
        </div>
        )}
