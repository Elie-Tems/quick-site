...
## קישור למודל ההפניות (חודש חינם)
מיגרציה `20260703110000_referral_free_month.sql` + הקוד: כשמופנה (`profiles.referred_by`)
הופך למשלם, `billing-icount-ipn` מזכה את **המפנה** ב-`free_months_credit += 1` (פעם אחת
לכל מופנה - טבלת `referral_rewards` עם unique מונעת כפילות). הקרון `billing-charge-run`
**צורך** קרדיט חודש-חינם לפני חיוב: מדלג על `cc/bill`, רושם חיוב ₪0, ומאריך את התקופה.
תואם להבטחה במייל siteReady ("על כל חבר שמצטרף והופך למנוי - חודש חינם").
