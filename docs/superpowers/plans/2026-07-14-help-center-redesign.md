...
**Goal:** Replace the cluttered two-panel Help Center (KB grid + always-visible chat card) with a unified search/chat bar, suggestion chips, and a category accordion — with AI chat appearing conditionally below only after messages are sent.

**Architecture:** Single file rewrite of `src/pages/HelpCenter.tsx` — all streaming logic, conversation memory, and `HARDCODED_ANSWERS` are preserved unchanged. The only structural change is merging `kbQuery` into `input` and replacing the 2-column KB grid with a vertical accordion controlled by `openCategory: string | null`. `knowledgeBase.ts` gets emoji strings in the `icon` field instead of Lucide component names.

**Tech Stack:** React, TypeScript, Tailwind CSS, framer-motion, shadcn/ui, ReactMarkdown, Supabase
...
| File | Change |
|---|---|
| `src/lib/knowledgeBase.ts` | Update 8 `icon` fields from Lucide names → emoji strings |
| `src/pages/HelpCenter.tsx` | Full JSX rewrite (logic preserved, layout replaced) |

---

### Task 1: Update knowledgeBase.ts icon fields to emoji

**Files:**
- Modify: `src/lib/knowledgeBase.ts`

- [ ] **Step 1: Update the `icon` comment and all 8 category icon values**

Replace the entire file's `KbCategory` interface comment and icon values. Only the `icon` field changes — nothing else in the file moves.

In `src/lib/knowledgeBase.ts`, make these targeted edits:

Change the interface comment:
```ts
// OLD:
...
Change each category's `icon` value:
```ts
// id: "start"     →  icon: "🚀"
// id: "products"  →  icon: "🛍️"
// id: "orders-customers"  →  icon: "📦"
// id: "design"    →  icon: "🎨"
// id: "marketing" →  icon: "📢"
// id: "payments-shipping"  →  icon: "💳"
// id: "verticals" →  icon: "📅"
// id: "account"   →  icon: "⚙️"
```

After the edit the `KNOWLEDGE_BASE` array head should look like:
```ts
...
Expected: no TypeScript errors (the `icon: string` type still matches emoji strings).

- [ ] **Step 3: Commit**

```powershell
git add src/lib/knowledgeBase.ts
git commit -m "refactor(help-center): replace lucide icon names with emoji in knowledgeBase"
```

---

### Task 2: Rewrite HelpCenter.tsx with new layout

**Files:**
- Modify: `src/pages/HelpCenter.tsx` (full replacement)

- [ ] **Step 1: Write the complete new file**

Replace the entire content of `src/pages/HelpCenter.tsx` with:
...
**מועד החיוב:**
- החיוב החודשי מתבצע **בכל חודש בתאריך שבו הצטרפתם** (הוראת קבע). הצטרפתם ב-15 לחודש? תחויבו ב-15 בכל חודש.
- **החודש הראשון** משולם במלואו בעת פרסום האתר, והאתר עולה מיד. החיוב הבא יהיה באותו תאריך בחודש הבא - חודש שלם.
...
- [ ] **Step 2: Verify TypeScript build passes (0 errors)**
...
Open `http://localhost:5173/help` and verify:

1. Page loads with "איך אפשר לעזור? 🤗" title and subtitle
2. Big search bar is visible with 💬 emoji and "שלח" button
3. Four suggestion chips appear below the bar
4. Divider "או עיין לפי נושא" and 8 accordion categories appear below chips
5. Click a category → it expands with articles; click another → first closes, second opens
6. Click an article → answer expands inline with markdown rendered
7. Type in search bar → chips disappear, accordion disappears, search results appear
8. Search for something with no matches → "לא מצאנו תשובה" + "שלח את השאלה לבוט" button appears
9. Click suggestion chip → chat panel appears below with messages; panel is NOT visible before any message is sent
10. "איפוס" button in chat header clears messages and panel disappears
11. Clear the search bar → chips and accordion return

- [ ] **Step 4: Commit**

```powershell
git add src/pages/HelpCenter.tsx
git commit -m "redesign(help-center): unified search/chat bar + category accordion, conditional chat panel"
```

- [ ] **Step 5: Push to trigger Cloudflare Pages deploy**
